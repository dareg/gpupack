...
# user packs use the value of the main pack if any ; already-compiled main packs as well ; unless forced by option 2 :
    if [ -d $MKTOP/$GMKMAIN ] ; then
      CPP_INTFB_CONTROL_FILE=$(find $(dirname $(ls -l $MKTOP/$GMKMAIN 2>/dev/null | $AWK '{print $NF}')) -name ".cpp_intfb" -type f)
    else
      unset CPP_INTFB_CONTROL_FILE
    fi
